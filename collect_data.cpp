...
static std::ofstream cOutputFile;
...
// static bool BuzzFetchX(buzzvm_t t_vm, int j) {
//    buzzobj_t e = BuzzTableGet(t_vm, j);
//    if(!e) {
//       THROW_ARGOSEXCEPTION("Robot \"fb" << t_vm->robot << "\"'s x[" << j << "] does not exist");
//    }
//    if(!buzzobj_isfloat(e)) {
//       THROW_ARGOSEXCEPTION("Robot \"fb" << t_vm->robot << "\"'s x[" << j << "] is not a float");
//    }
//    float x = buzzobj_getint(e);
//    // if(xij < 0 || xij > 1) {
//    //    THROW_ARGOSEXCEPTION("Robot \"fb" << t_vm->robot << "\"'s px[" << j << "] is not 0 or 1");
//    // }
//    return x;
// }

// static Real BuzzFetchY(buzzvm_t t_vm, int j) {
//    buzzobj_t e = BuzzTableGet(t_vm, j);
//    if(!e) {
//       THROW_ARGOSEXCEPTION("Robot \"fb" << t_vm->robot << "\"'s y[" << j << "] does not exist");
//    }
//    if(!buzzobj_isfloat(e)) {
//       THROW_ARGOSEXCEPTION("Robot \"fb" << t_vm->robot << "\"'s y[" << j << "] is not a float");
//    }
//    float y = buzzobj_getfloat(e);
//    // if(yij < 0.0) {
...
//       /* Make sure 'px' and 'py' exist and are vectors */
//       BuzzAssertVector(t_vm, "px");
//       BuzzAssertVector(t_vm, "py");
//       /* Extract data from px */
//       BuzzTableOpen(t_vm, "px");
//       for(size_t j = 0; j < m_nTasks; ++j) {
//          sAssignment.px[j] = BuzzFetchX(t_vm, j);
//       }
//       BuzzTableClose(t_vm);
//       /* Extract data from py */
//       BuzzTableOpen(t_vm, "py");
//       for(size_t j = 0; j < m_nTasks; ++j) {
//          sAssignment.py[j] = BuzzFetchY(t_vm, j);
//       }
//       BuzzTableClose(t_vm);
...
//    virtual void operator()(const std::string& str_robot_id,
//                            buzzvm_t t_vm) {
//       /* Set the values  the table 'tasks' in the Buzz VM */
...
void CCollectData::Init(TConfigurationNode& t_tree) {
   cOutputFile.open("data.dat", std::ios::out);
   cOutputFile << "timestep,robot_id,x,y,yaw\n";
...
void CCollectData::Reset() {
   if (cOutputFile.is_open()) {
      cOutputFile.close();
      cOutputFile.open("data.dat", std::ios::out);
      cOutputFile << "timestep,robot_id,px,py\n";
   }
...
// CColor CCOLLECTDATA::GetFloorColor(const CVector2& c_position_on_plane) {
...
void CCollectData::PostStep() {
   // size_t timestep = GetSpace().GetSimulationClock();
   size_t timestep = argos::CSimulator::GetInstance().GetSpace().GetSimulationClock();


   // Loop over all KheperaIV robots
   // CSpace::TMapPerType& tKheperaMap = GetSpace().GetEntitiesByType("kheperaiv");
   CSpace::TMapPerType& tKheperaMap = argos::CSimulator::GetInstance().GetSpace().GetEntitiesByType("kheperaiv");

   // Loop over all KheperaIV robots
   // Remember that the map is a std::map, so we can use iterators
   // to loop over the elements
   // References:
   //   - https://github.com/ilpincy/argos3-examples
   //   - https://github.com/ilpincy/argos3-examples/blob/master/loop_functions/trajectory_loop_functions/trajectory_loop_functions.cpp#L68
   //   - https://github.com/ilpincy/argos3-examples/blob/master/loop_functions/foraging_loop_functions/foraging_loop_functions.cpp#L125
   for (auto& it : tKheperaMap) {
      CKheperaIVEntity& cKhepera = *any_cast<CKheperaIVEntity*>(it.second);
      const CVector3& pos = cKhepera.GetEmbodiedEntity().GetOriginAnchor().Position;

      const CQuaternion& orient = cKhepera.GetEmbodiedEntity().GetOriginAnchor().Orientation;

      CRadians cYaw, cPitch, cRoll;
      orient.ToEulerAngles(cYaw, cPitch, cRoll);

      std::string id = cKhepera.GetId();  // e.g., "k0", "k1", ...
      cOutputFile << timestep << "," << id << "," << pos.GetX() << "," << pos.GetY() << "," << cYaw.GetValue() << "\n";
   }
   ...
   //    /* Robot id */
   //    m_cOutFile << "\t" << i->first;
   //    /* Values of pose.position.x */
   //    for(size_t j = 0; j < i->second.px.size(); ++j)
   //       m_cOutFile << "\t" << i->second.px[j];
   //    /* Values of pose.position.y */
   //    for(size_t j = 0; j < i->second.py.size(); ++j)
   //       m_cOutFile << "\t" << i->second.py[j];
   //    m_cOutFile << std::endl;
   // }
}
...
bool CCollectData::IsExperimentFinished() {
...
// int CCOLLECTDATA::GetNumRobots() const {
...
// void CCOLLECTDATA::BuzzBytecodeUpdated() {
...
// std::vector<STask> CCOLLECTDATA::GetTasks() const {
//    return m_vecTasks;
// }
...
REGISTER_LOOP_FUNCTIONS(CCollectData, "collect_data");
